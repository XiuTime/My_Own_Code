...
{
    if (!_arrowImage) {
        UIImageView* arrowImage = [[UIImageView alloc] initWithImage:[UIImage imageNamed:MJRefreshSrcPath]];
...
- (void)layoutSubviews
{
    ...
    // 1.箭头 箭头被取消，换成了自己的loading
//    CGFloat arrowX = self.mj_width * 0.5 - 80;
//    self.arrowImage.center = CGPointMake(arrowX, self.mj_height * 0.5);
    self.arrowImage.center = CGPointMake(SCREEN_WIDTH/2, self.mj_height /2);
    
    
    // 2.指示器
//    self.activityView.center = self.arrowImage.center;
//    self.activityView.center = CGPointMake(SCREEN_WIDTH/2, self.mj_height /2);
}
...
//        rotationAnimation.duration = MJRefreshSlowAnimationDuration;
...
//        DVLog(@"------post refresh method %@", NSStringFromSelector(self.beginRefreshingAction));
        // 回调
        if ([self.beginRefreshingTaget respondsToSelector:self.beginRefreshingAction]) {
            msgSend(msgTarget(self.beginRefreshingTaget), self.beginRefreshingAction, self);
        }
...
- (void)startAnimation
{
    ...
    [UIView animateWithDuration:0.01 delay:0 options:UIViewAnimationOptionCurveLinear animations:^{
        self.arrowImage.transform = endAngle;
    } completion:^(BOOL finished) {
        if (stopAnimation) {
            self.arrowImage.hidden = YES;
            return;
