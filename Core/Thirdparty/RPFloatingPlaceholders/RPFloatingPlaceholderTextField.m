...
{
    if ([self.cachedPlaceholder isEqualToString:aPlaceholder])
        return;
...
- (void)setupViewDefaults
{
    ...
    self.shouldFloatWhenStartEdit = NO;
...
- (void)drawRect:(CGRect)aRect
{
    ...
    if (self.shouldDrawPlaceholder) {
        ...
        //        CGRect placeholderFrame = CGRectMake(5.f, floorf((self.frame.size.height - self.font.lineHeight) / 2.f + 20), self.frame.size.width - 10, self.frame.size.height);
        CGRect placeholderFrame = CGRectMake(5.f, 20 + 8, self.frame.size.width - 10, self.frame.size.height);
        //        placeholderFrame = self.originalTextFieldFrame;
...
- (void)adjustFramesForNewPlaceholder
{
    ...
    self.offsetFloatingLabelFrame = CGRectMake(self.originalFloatingLabelFrame.origin.x, self.originalFloatingLabelFrame.origin.y - offset + 20,
...
{
    return [super textRectForBounds:UIEdgeInsetsInsetRect(bounds, UIEdgeInsetsMake(20.f, 5.f, 0.f, 5.f))];
}

// Adds padding so these text fields align with RPFloatingPlaceholderTextView's
- (CGRect)editingRectForBounds:(CGRect)bounds
{
    return [super editingRectForBounds:UIEdgeInsetsInsetRect(bounds, UIEdgeInsetsMake(20.f, 5.f, 0.f, 5.f))];
}

//- (CGRect)placeholderRectForBounds:(CGRect)bounds
//{
//    return [super editingRectForBounds:UIEdgeInsetsInsetRect(bounds, UIEdgeInsetsMake(40.f, 5.f, 0.f, 5.f))];
//}
...
- (void)animateFloatingLabelColorChangeWithAnimationBlock:(void (^)(void))animationBlock
{
    ...
    [UIView transitionWithView:self.floatingLabel
                      duration:0.25
                       options:options
                    animations:^{
                        animationBlock();
...
{
    BOOL previousShouldDrawPlaceholderValue = self.shouldDrawPlaceholder;
    self.shouldDrawPlaceholder = (!self.hasText) & (!self.shouldFloatWhenStartEdit);
