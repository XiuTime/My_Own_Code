...
- (void)dv_scrollToItemAtIndex:(NSInteger)index animated:(BOOL)animated
{
    CGFloat aimOffset = [self minScrollDistanceFromOffset:_scrollOffset toOffset:index];

    [self disableApp];
    [UIView animateWithDuration:SCROLL_DURATION delay:0 options:UIViewAnimationOptionCurveEaseInOut animations:^{
        self.scrollOffset += aimOffset;
    }
...
{
    return (fabs(_startVelocity) > SCROLL_SPEED_THRESHOLD) && (fabs([self decelerationDistance]) > DECELERATE_THRESHOLD);
}

- (BOOL)shouldScroll
{
    return (fabs(_startVelocity) > SCROLL_SPEED_THRESHOLD) && (fabs(_scrollOffset - self.currentItemIndex) > SCROLL_DISTANCE_THRESHOLD);
}
    ...
    - (BOOL)gestureRecognizer : (UIGestureRecognizer*)gesture shouldReceiveTouch : (UITouch*)touch
{
    ...
    if ([gesture isKindOfClass:[UITapGestureRecognizer class]]) {
        ...
        if (!_scrollEnabled) {
            ...
            if (scrollView) {
                return !scrollView.scrollEnabled || (self.vertical && scrollView.contentSize.height <= scrollView.frame.size.height) || (!self.vertical && scrollView.contentSize.width <= scrollView.frame.size.width);
            }
...
//        if ([gesture.view isKindOfClass:[UIControl class]] && ![gesture.view isKindOfClass:[UIScrollView class]]) {
//            return NO;
//        }
...
- (void)didTap:(UITapGestureRecognizer*)tapGesture
{
    ...
    if (index != NSNotFound) {
        if (!_delegate || [_delegate carousel:self shouldSelectItemAtIndex:index]) {
            if ((index != self.currentItemIndex && _centerItemWhenSelected) || (index == self.currentItemIndex && _scrollToItemBoundary)) {
